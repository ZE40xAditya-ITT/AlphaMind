    ...
    <div className="overflow-x-auto rounded-2xl border border-slate-800">
      <table className="w-full text-left text-sm">
        <thead className="bg-[#111827] border-b border-slate-800">
          <tr>
            <th className="p-4 font-semibold text-slate-400 uppercase text-xs tracking-wider">Asset</th>
            <th className="p-4 font-semibold text-slate-400 uppercase text-xs tracking-wider">Quantity</th>
            <th className="p-4 font-semibold text-slate-400 uppercase text-xs tracking-wider">Avg Buy Price</th>
            <th className="p-4 font-semibold text-slate-400 uppercase text-xs tracking-wider">Current Price</th>
            <th className="p-4 font-semibold text-slate-400 uppercase text-xs tracking-wider">Total Invested</th>
            <th className="p-4 font-semibold text-slate-400 uppercase text-xs tracking-wider">Current Value</th>
            <th className="p-4 font-semibold text-slate-400 uppercase text-xs tracking-wider">Returns</th>
            <th className="p-4 font-semibold text-slate-400 uppercase text-xs tracking-wider text-right">Actions</th>
          </tr>
        </thead>
        <tbody className="divide-y divide-slate-800 bg-[#0B1121]">
            ...
            stocksToDisplay.map((stock: any, index: number) => (
              ...
              >
                ...
                <td className="p-4 text-right flex justify-end gap-2">
                  <button 
                    onClick={() => {
                      const amount = window.prompt(`How many shares of ${stock.symbol} would you like to sell/reduce? (Current: ${stock.quantity})`);
                      if (amount && !isNaN(parseFloat(amount)) && parseFloat(amount) > 0) {
                        handleReduceStock(stock.id, parseFloat(amount));
                      }
                    }} 
                    className="p-2 text-slate-400 hover:text-amber-500 hover:bg-amber-500/10 rounded-lg transition text-xs font-semibold" 
                    title="Reduce Stock"
                  >
                    Reduce
                  </button>
                  <button onClick={() => handleRemoveStock(stock.id)} className="p-2 text-slate-400 hover:text-rose-500 hover:bg-rose-500/10 rounded-lg transition" title="Remove Stock">
                    <Trash2 size={16} />
                  </button>
                </td>
              </motion.tr>
